...
import finishReservation from '@salesforce/apex/ReservationController.finishReservationByReader';
import getBooks from '@salesforce/apex/BookController.getBorrowedBookReservationsByOwnerId';
...
export default class BorrowedBooks extends LightningElement {
  ...
  async handleOkReservationModal(event) {
    ...
    if (this.overviewIsOpen) {
      this.overviewRecord = this.records.find(item => item.Id === event.detail.reservation.Book__c);
    }
